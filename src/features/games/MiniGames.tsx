...
import { Sparkles, Trophy, RotateCcw, Heart, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const MiniGames: React.FC<MiniGamesProps> = ({ content, onComplete, accentColor = '#b45309' }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto my-6 p-6 rounded-3xl bg-[#fffdfa] border border-amber-200/70 shadow-sm">
      <div className="text-center mb-6">
        <h3 className="text-2xl font-bold font-playfair text-stone-800">{content.title}</h3>
        <p className="text-sm text-stone-600 mt-1 max-w-md mx-auto">{content.instructions}</p>
      </div>
...
const PuzzleGame: React.FC<{ content: MiniGameContent; onComplete?: () => void; accentColor: string }> = ({
  ...
  const defaultImage = content.puzzleImageUrl || 'https://images.unsplash.com/photo-1513151233558-d860c5398176?w=600&auto=format&fit=crop&q=80';
