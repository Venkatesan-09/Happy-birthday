...
export const ExperienceRenderer: React.FC<ExperienceRendererProps> = ({
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-b ${experience.theme?.background || 'from-[#faf6f0] to-[#f0e4d0]'} text-[#2c2623] pb-24 transition-colors duration-500`}>
      ...
      <div className="fixed top-4 right-4 z-50 flex items-center gap-2 bg-white/90 backdrop-blur-md px-3.5 py-2 rounded-full border border-amber-200/80 shadow-md">
        <button
          onClick={toggleSoundtrack}
          className="flex items-center gap-1.5 text-xs font-medium text-amber-900 hover:text-amber-700 cursor-pointer"
        >
          {isPlayingAudio ? <Pause className="w-3.5 h-3.5 fill-current" /> : <Play className="w-3.5 h-3.5 fill-current" />}
          <span>{isPlayingAudio ? 'Soundtrack Playing' : 'Play Music'}</span>
        </button>
        <div className="w-px h-3 bg-amber-200" />
        <button
          onClick={toggleMute}
          className="text-stone-600 hover:text-amber-800 p-1 cursor-pointer"
...
const MusicModule: React.FC<{ content: any }> = ({ content }) => {
  ...
  return (
    <div className="p-6 rounded-3xl bg-[#fffdfa] border border-amber-200 flex items-center justify-between shadow-sm">
      ...
      <span className="text-xs px-3 py-1 rounded-full bg-amber-100 text-amber-800 font-medium">
        {playing ? 'Playing Ambient Sound' : 'Tap to Play'}
      </span>
    </div>
  );
};
    ...
    <div className="p-6 rounded-3xl bg-[#fffdfa] border border-amber-200 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-stone-500">From {content.senderName} • {content.durationSeconds}s</p>
        </div>
        <button
          onClick={playVoice}
...
const VideoModule: React.FC<{ content: any }> = ({ content }) => {
  return (
    <div className="p-6 rounded-3xl bg-[#fffdfa] border border-amber-200 shadow-sm text-center">
      <h4 className="font-playfair font-bold text-stone-900 text-xl">{content.title}</h4>
      <p className="text-xs text-stone-600 mt-1 mb-4">{content.description}</p>
      <div className="rounded-2xl overflow-hidden shadow-md max-w-xl mx-auto bg-black aspect-video flex items-center justify-center">
        {content.videoUrl ? (
          <video src={content.videoUrl} controls className="w-full h-full" poster={content.thumbnailUrl} />
        ) : (
          <p className="text-white text-sm">Video message</p>
        )}
      </div>
    </div>
  );
      ...
      {!unlocked ? (
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="mt-6 p-6 rounded-2xl bg-white border border-amber-300 text-left">
          ...
          {content.secretMediaUrl && (
            <img src={content.secretMediaUrl} alt="Secret" className="mt-4 rounded-xl max-h-60 w-full object-cover" referrerPolicy="no-referrer" />
          )}
        </motion.div>
      )}
    </div>
        ...
        {!opened ? (
          ...
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-md mx-auto p-6 rounded-3xl bg-amber-50 border-2 border-amber-400 shadow-md">
            ...
            {content.giftPhotoUrl && (
              <img src={content.giftPhotoUrl} alt="Gift" className="mt-4 rounded-xl w-full h-48 object-cover border border-amber-200" referrerPolicy="no-referrer" />
            )}
          </motion.div>
        )}
      </div>
