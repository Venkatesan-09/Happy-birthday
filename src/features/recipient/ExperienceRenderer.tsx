...
export const ExperienceRenderer: React.FC<ExperienceRendererProps> = ({
  ...
  // Resolve background MP3 audio URL: from experience.backgroundMusicUrl OR from MUSIC (Voice Note) OR VOICE module
  const audioNoteModule = modules.find(
    (m) => (m.type === 'MUSIC' || m.type === 'VOICE' || m.type === 'voice') && m.enabled && m.content?.audioUrl
  );
  const rawBgAudio =
    experience.backgroundMusicUrl ||
    (audioNoteModule?.content?.playAsBackground !== false ? audioNoteModule?.content?.audioUrl : null) ||
    audioNoteModule?.content?.audioUrl;
