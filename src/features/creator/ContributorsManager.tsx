      ...
      <div className="p-5 rounded-2xl bg-[#fffdfa] border border-stone-200 shadow-xs">
        ...
        {contributors.length === 0 ? (
          ...
          <div className="divide-y divide-stone-100">
            {contributors.map((c) => (
              <div key={c._id} className="py-3 flex items-center justify-between gap-2">
                <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {copiedToken === c.token ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3" />}
                    <span>{copiedToken === c.token ? 'Copied' : 'Copy Link'}</span>
                  </button>
                  <button
