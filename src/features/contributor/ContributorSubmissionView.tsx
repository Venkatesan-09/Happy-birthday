...
import { Heart, Send, CheckCircle2, Sparkles, Image as ImageIcon } from 'lucide-react';
...
export const ContributorSubmissionView: React.FC<ContributorSubmissionViewProps> = ({
  ...
  useEffect(() => {
    api.contributors.getByToken(token)
      .then((res) => {
        setData(res);
      })
      .catch((err) => {
  ...
  if (!data || !data.contributor) {
    return (
      <div className="min-h-screen bg-[#faf8f5] flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-xs text-stone-500 mt-1">This contributor invite link may have expired or is invalid.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#faf6f0] to-[#f0e4d0] flex items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-lg rounded-3xl bg-[#fffdfa] border border-amber-200 shadow-xl p-6 sm:p-8 text-center">
        <TeddyMascot pose={submitted ? 'celebrating' : 'holding_gift'} size="md" message={submitted ? 'Wish delivered!' : 'Add your love!'} />
        ...
        {!submitted ? (
          <>
            <div className="mt-3 space-y-1">
              ...
              <p className="text-xs text-stone-600 max-w-sm mx-auto">
                Hi {contributor.name}! Write a personal wish, memory, or loving message that will be featured in {experience?.recipient?.name}’s interactive birthday experience.
              </p>
            </div>
          ...
          <div className="mt-4 space-y-3">
            <h3 className="font-playfair font-bold text-2xl text-stone-900">Thank you, {contributor.name}!</h3>
            <p className="text-xs text-stone-600 max-w-sm mx-auto">
              Your message has been safely sealed into {experience?.recipient?.name}’s interactive birthday experience. They will get to experience it on their special day!
            </p>
          </div>
        )}
      </div>
